...
Validation, Security, Authentication, Concurrency, Data, JSON Processing,
JSON Binding, and Debugging Support. Run one at a time with:
...
per-specification status: Annotations, Dependency Injection, JSON Processing
(197/197 on Apache Johnzon), and Debugging Support pass, and every other
suite — including the source-reactor TCKs (Security, Authentication, Faces)
— executes end to end with dated result counts and the remaining TomEE or
provider gaps recorded for triage.
