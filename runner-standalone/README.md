...
| CDI 4.1 (core) + Interceptors 2.2 (non-EJB half) | `jakarta.enterprise:cdi-tck-core-impl:4.1.0` | `cdi` | Harness validated (trial class 19/19); full web-profile suite run in progress — record results in EXPECTED_FAILURES once triaged |
| CDI 4.1 (EE integration) + Interceptors 2.2 (EJB half) | `jakarta.tck:cdi-tck-ee-impl:11.0.3` | `cdi-ee` | Configured, not yet run |
| Concurrency 3.1 | `jakarta.enterprise:jakarta.enterprise.concurrent-tck:3.1.1` | `concurrency` | Runs (187 tests: 92 pass / 46 fail / 49 error / 14 skipped, 2026-07-18). Signature and virtual-thread tests pass; the failures are TomEE Concurrency 3.1 implementation gaps to triage |
...
| Servlet 6.1 | EFTL zip (installed as `jakarta.tck:servlet-tck-runtime:6.1.0`) | `servlet` | Configured, not yet run. Known gaps: client-cert tests need a TomEE vendor descriptor (reference runner injects sun-web.xml principal mapping via an Arquillian archive processor), and tests targeting the dedicated https deployment need a second container definition |
| Validation 3.1 | EFTL zip (installed as `jakarta.validation:validation-tck-tests:3.1.1`) | `validation` | Configured (Apache BVal provider), not yet run. Note: the reference runner warns the TCK misbehaves when the runner pom has a Maven parent — if no tests execute, that inheritance quirk is the first suspect |
