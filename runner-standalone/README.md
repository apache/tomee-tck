...
| Servlet 6.1 | EFTL zip (installed as `jakarta.tck:servlet-tck-runtime:6.1.0`) | `servlet` | **Runs: 1,706 tests, 71 errors, 7 skipped** (2026-07-18) once slf4j-simple resolves at the TCK-derived version. The runner trusts the TCK's bundled client certificate, maps its identity through an injected sun-web.xml, and defines the http/https container group; remaining errors are behavioral differences (async dispatch connection handling, response-content mismatches) to triage as product results |
...
| Authentication 3.1 | Source reactor zip 3.1.2 | `authentication` (Maven module) | Same download-and-patch runner as security, using Tomcat's AuthConfigFactory for the spi unit tests; Web Profile scope (EJB/JACC/SOAP modules skipped). First run in progress |
| Faces 4.1 | Source reactor zip 4.1.2 | `faces` (Maven module) | Same download-and-patch runner covering the modern Arquillian modules (faces22/23/40/41, ~319 tests) on Plume's Mojarra. The ~5,500-test legacy JavaTest `old-tck` half is GlassFish-wired (asadmin deployment) and remains the one open port, as does the GlassFish-bound faces-signaturetest module |
