...
`TOMEE_CLASSIFIER` selects the distribution, default `plume`; it accepts
`plume`, `webprofile`, `microprofile`, and `plus`, and the script rejects
anything else. The three source-reactor runners (`security`, `authentication`,
`faces`) carry it into the `tomee-remote` profile they inject into the
downloaded TCK, so it reaches those suites too. The recorded result counts
below were taken on the distribution each row names; apache/tomee validates
the Security TCK against Plus.
...
The default Maven build compiles these modules but runs nothing; execution
requires `-Dtck.standalone=true` (the script passes it).
